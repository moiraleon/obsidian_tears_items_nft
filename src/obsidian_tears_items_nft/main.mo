...
actor class ObsidianTearsItems() = this {
  ...
  */
  ...
  var teamNftAddress : AccountIdentifier = "57e8ee37eb3a7b82a859e9f804da939160b7abad6e4656eed999c167de233f90"; // item nft wallet
  ...
  stable var publicSaleStart : Time = 0; //In nanoseconds. Start of first purchase (WL or other)
  stable var whitelistTime : Time = 0; //In nanoseconds. When WL ends. Set to publicSaleStart for no exclusive period
  stable var marketDelay : Time = 0; //In nanoseconds. How long to delay market opening
  stable var whitelistOneTimeOnly : Bool = false; //Whitelist addresses are removed after purchase
  stable var whitelistDiscountLimited : Bool = false; //If the whitelist discount is limited to the whitelist period only. If no whitelist period this is ignored
  stable var nftCollectionName : Text = "Obsidian Tears Items";
  stable var imageWidth : Text = "300"; //size of full size
  stable var imageHeight : Text = imageWidth; //size of full size
  stable var imageType : Text = "image/svg+xml"; //type of thumbnails
  stable var whitelistLimit : Nat = 1; //initial whitelist
  stable var initialWhitelist : [AccountIdentifier] = []; //initial whitelist
  stable var _launchPrepped : Bool = false; //have set launch variables
                      ...
                      case (?t) {
                        if (t == "thumbnail") {
                          ...
                          return {
                            ...
                            body = _getHttpBody(data, height, width);
                            streaming_strategy = null;
                          };
                        };
                      };
  ...
  public shared (msg) func prepLaunch(
    ...
    new_marketDelay : Time,
    new_whitelistOneTimeOnly : Bool,
    new_whitelistDiscountLimited : Bool,
    new_nftCollectionName : Text,
    ...
    airdrop := new_airdrop;
    initialWhitelist := new_initialWhitelist;
    marketDelay := new_marketDelay;
    nftCollectionName := new_nftCollectionName;
    publicSaleStart := new_publicSaleStart;
    reservedAmount := new_reservedAmount;
    whitelistDiscountLimited := new_whitelistDiscountLimited;
    whitelistLimit := new_whitelistLimit;
    whitelistOneTimeOnly := new_whitelistOneTimeOnly;
    whitelistPrice := new_whitelistPrice;
    whitelistTime := new_whitelistTime;
    _launchPrepped := true;
  };
};
